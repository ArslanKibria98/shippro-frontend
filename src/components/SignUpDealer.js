...
const SignUpDealer = ({ onSignupSuccess }) => {
   const [name, setName] = useState("");
   const [email, setEmail] = useState("");
   const [password, setPassword] = useState("");
   const { user, signupdealer } = useContext(AuthContext);
      ...
      try {
         await toast.promise(
            new Promise(async (resolve, reject) => {
               const success = await axios.post(`${process.env.REACT_APP_API_URL}/api/auth/dealer/${user?.id}/add-subuser`, { name: name, email: email, password: password });
