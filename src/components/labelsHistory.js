...
import DownloadBulkHistoryPdf from "./DownloadBulkHistoryPdf";
...
import redirect from "../components/Images/images.png"
const LabelsHistory = () => {
  ...
  const groupTrackingNumbers = (trackingNumbers) => {
    ...
    for (let i = 0; i < trackingNumbers.length; i += chunkSize) {
      ...
      console.log(group, "chunks")
      groups.push(group);
    }
    console.log(groups, trackingNumbers, "trackingGroups")
    return groups;
  };
  ...
  const handleBulkTrack = (trackingNumbers) => {
    ...
    console.log(trackingNumbersString, trackingNumbers, "trackingNumbersStringtrackingNumbersStringtrackingNumbersString")
    window.open(`https://t.17track.net/en#nums=${trackingNumbersString}`);
  };
                        ...
                        {currentBulkHistory?.map((event, bulkIndex) => {
                          ...
                          return (
                            <tr key={event._id || bulkIndex}>
                              <td>{event.labels[0].fileName}</td>
                              <td>{event.labels.length}</td>
                              <td>
                                <span style={{ background: "green", borderRadius: "10px", color: "#fff", padding: "10px" }}>Ready</span>
                              </td>
                              <td>{event.labels[0].vendor}</td>
                              <td>{new Date(event.generatedAt).toLocaleString()}</td>
                              <td>
                                {trackingGroups.map((group, index) => (
                                  <div key={index} className="tracking-group d-flex align-items-center">

                                    <span>{index === 0 ? "1-20" : `${index * 20 + 1}-${Math.min((index + 1) * 20, event.labels.length)}`}</span>
                                    <img
                                      src={redirect}  // Replace with actual image source
                                      alt={`Tracking Group ${index + 1}`}
                                      className="tracking-image cursor-pointer"
                                      onClick={() => handleBulkTrack(group)}
                                      style={{ width: "10px", height: "10px", cursor: "pointer", marginLeft: "10px" }}
                                    />
                                  </div>
                                ))}
                              </td>
                              <td style={{ display: 'flex' }} className="gap-2">
                                <div className="d-flex gap-2">
                                  ...
                                  <DownloadBulkHistoryPdf labelDataList={event.labels} file={event.labels[0].fileName} />
                                  <DownloadBulkHistory labelDataList={event.labels} file={event.labels[0].fileName} />
                                </div>
                              </td>
                            </tr>
                          );
