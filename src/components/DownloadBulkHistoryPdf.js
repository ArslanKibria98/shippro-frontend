...
const DownloadBulkHistoryPdf = ({ labelDataList, file }) => {
   const labelRefs = useRef([]);
   const [isDownloading, setIsDownloading] = useState(false);
   const [downloadProgress, setDownloadProgress] = useState(0);

   const generatePDF = async (refElement) => {
      const options = {
         margin: [0, 0, 0, 0],
         html2canvas: {
            scale: 3,
   ...
   const downloadMergedPdf = async () => {
      ...
      try {
         for (let i = 0; i < labelDataList.length; i++) {
            const pdfBlob = await generatePDF(labelRefs.current[i]);
            pdfBlobs.push(pdfBlob);
            setDownloadProgress(Math.round(((i + 1) / labelDataList.length) * 100));
         }

         if (pdfBlobs.length > 0) {
            const mergedPdfBytes = await mergePDFs(pdfBlobs);
            saveAs(new Blob([mergedPdfBytes], { type: "application/pdf" }), `${file}_Merged.pdf`);
         }
      } catch (error) {
...
export default DownloadBulkHistoryPdf;
